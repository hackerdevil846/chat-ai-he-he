...
module.exports = {
    ...
    onStart: async function({ message, args, event }) {
        ...
        if (args[0] && !isNaN(args[0])) {
            ...
            try {
                ...
                return message.reply(`${doya.title}\n\n${doya.body}`);
            }
        }
    ...
    onReply: async function({ event, message, Reply }) {
        try {
            ...
            const { doyaContent, currentPage, itemsPerPage } = Reply;
            const totalPages = Math.ceil(doyaContent.length / itemsPerPage);
            
            if (event.body.toLowerCase() === 'next' && currentPage < totalPages) {
                // Show next page
                const startIndex = currentPage * itemsPerPage;
                const endIndex = Math.min(startIndex + itemsPerPage, doyaContent.length);
                
                let menuMessage = `📖 ইসলামিক দোয়া সংগ্রহ (পৃষ্ঠা ${currentPage + 1}/${totalPages}):\n\n`;
                for (let i = startIndex; i < endIndex; i++) {
                    menuMessage += `${i + 1}. ${doyaContent[i].title}\n`;
                }
                
                menuMessage += `\n💫 আপনার পছন্দের দোয়ার নম্বরটি লিখুন\n💡 'next' লিখলে পরের পৃষ্ঠা দেখাবে`;
                
                const msg = await message.reply(menuMessage);
                
                // Update reply data
                Reply.messageID = msg.messageID;
                Reply.currentPage = currentPage + 1;
                
            } else if (event.body.toLowerCase() === 'prev' && currentPage > 1) {
                // Show previous page
                const newPage = currentPage - 1;
                const startIndex = (newPage - 1) * itemsPerPage;
                const endIndex = Math.min(startIndex + itemsPerPage, doyaContent.length);
                
                let menuMessage = `📖 ইসলামিক দোয়া সংগ্রহ (পৃষ্ঠা ${newPage}/${totalPages}):\n\n`;
                for (let i = startIndex; i < endIndex; i++) {
                    menuMessage += `${i + 1}. ${doyaContent[i].title}\n`;
                }
                
                menuMessage += `\n💫 আপনার পছন্দের দোয়ার নম্বরটি লিখুন\n💡 'prev' লিখলে আগের পৃষ্ঠা দেখাবে`;
                
                const msg = await message.reply(menuMessage);
                
                // Update reply data
                Reply.messageID = msg.messageID;
                Reply.currentPage = newPage;
                
            } else if (!isNaN(event.body)) {
                ...
                await message.reply("⚠️ দয়া করে একটি বৈধ নম্বর লিখুন (1-8) অথবা 'next'/'prev' লিখুন");
            }
            
        } catch (error) {
            console.error("দোয়া রিপ্লাই ত্রুটি:", error);
