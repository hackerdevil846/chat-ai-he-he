...
module.exports = {
    ...
    onStart: async function({ message, event, args, usersData, getText }) {
        try {
            ...
            switch (args[0]) {
                ...
                case "-a": {
                    ...
                    return message.reply(getText("listAdmin", msg.join("\n") || "𝑁𝑜 𝑎𝑑𝑚𝑖𝑛𝑠 𝑓𝑜𝑢𝑛𝑑"));
                }

                case "add": {
                    // Check if user is bot admin
                    if (!config.ADMINBOT.includes(event.senderID.toString())) {
                        return message.reply(getText("notHavePermssion", "𝑎𝑑𝑑"));
                    }
                    ...
                    if (mention.length > 0) {
                        ...
                        return message.reply(getText("addedNewAdmin", mention.length, listAdd.join("\n")));
                    }
                        ...
                        if (!config.ADMINBOT.includes(targetID)) {
                            ...
                            try {
                                ...
                                const name = userInfo.name || "𝑈𝑛𝑘𝑛𝑜𝑤𝑛 𝑈𝑠𝑒𝑟";
                                fs.writeFileSync(configPath, JSON.stringify(config, null, 4), 'utf8');
                                return message.reply(getText("addedNewAdmin", 1, `[ ${targetID} ] » ${name}`));
                            } catch (error) {
                                fs.writeFileSync(configPath, JSON.stringify(config, null, 4), 'utf8');
                                return message.reply(getText("addedNewAdmin", 1, `[ ${targetID} ] » 𝑈𝑛𝑘𝑛𝑜𝑤𝑛 𝑈𝑠𝑒𝑟`));
                            }
                        } else {
                ...
                case "delete": {
                    // Check if user is bot admin
                    if (!config.ADMINBOT.includes(event.senderID.toString())) {
                        return message.reply(getText("notHavePermssion", "𝑑𝑒𝑙𝑒𝑡𝑒"));
                    }
                    ...
                    if (mention.length > 0) {
                        ...
                        fs.writeFileSync(configPath, JSON.stringify(config, null, 4), 'utf8');
                        return message.reply(getText("removedAdmin", mention.length, listRemove.join("\n")));
                    }
                    else if (args[1] && !isNaN(args[1])) {
                        const targetID = args[1];
                        ...
                        if (index !== -1) {
                            ...
                            try {
                                ...
                                const name = userInfo.name || "𝑈𝑛𝑘𝑛𝑜𝑤𝑛 𝑈𝑠𝑒𝑟";
                                fs.writeFileSync(configPath, JSON.stringify(config, null, 4), 'utf8');
                                return message.reply(getText("removedAdmin", 1, `[ ${targetID} ] » ${name}`));
                            } catch (error) {
                                fs.writeFileSync(configPath, JSON.stringify(config, null, 4), 'utf8');
                                return message.reply(getText("removedAdmin", 1, `[ ${targetID} ] » 𝑈𝑛𝑘𝑛𝑜𝑤𝑛 𝑈𝑠𝑒𝑟`));
                            }
                        } else {
