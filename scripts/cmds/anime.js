module.exports = {
    config: {
        ...
        guide: {
            en: "{prefix}anime [genre]\nAvailable genres: shonen, seinen, isekai, scifi"
        }
    },
    ...
    onStart: async function ({ api, args, message, event }) {
        ...
        if (args.length === 0) {
            ...
            return message.reply(`❌ | Please specify a genre!\nAvailable genres: ${genreList}\nExample: anime shonen`);
        }
        ...
        if (!animeRecommendations[genre]) {
            const genreList = Object.keys(animeRecommendations).join(', ');
            return message.reply(`❌ | Invalid genre! Available genres: ${genreList}`);
        }

        const loadingMsg = await message.reply("⏳ | Loading anime recommendation...");

        try {
            const recommendations = animeRecommendations[genre];
            let randomIndex;
            
            // Get random index that's different from last recommendation
            do {
            ...
            await message.reply("❌ | Failed to load anime recommendation. Please try again later.");
            await api.unsendMessage(loadingMsg.messageID);
        }
    }
};
