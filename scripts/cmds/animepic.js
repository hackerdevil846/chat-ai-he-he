module.exports = {
  ...
  onStart: async function ({ message, event, args }) {
    ...
    try {
      ...
      const animeDataPath = resolve(process.cwd(), 'scripts', 'cmds', 'cache', 'anime.json');
      const animeData = JSON.parse(readFileSync(animeDataPath));
      ...
      if (args.length === 0 || !animeData.hasOwnProperty(args[0])) {
        ...
        return await message.reply(`=== 𝐴𝑙𝑙 𝐴𝑣𝑎𝑖𝑙𝑎𝑏𝑙𝑒 𝐴𝑛𝑖𝑚𝑒 𝑇𝑎𝑔𝑠 ===\n${list.join(", ")}`);
      }
  ...
  getAnime: async function (type) {
    ...
    try {
      ...
      // Save temporary image to the same cache directory
      const path = resolve(process.cwd(), 'scripts', 'cmds', 'cache', `${string}.${ext}`);
  ...
  onLoad: async function () {
    const { resolve } = global.nodemodule["path"];
    const { existsSync, readFileSync, ensureDirSync, writeFileSync } = global.nodemodule["fs-extra"];

    // Use the custom cache path
    const cacheDir = resolve(process.cwd(), 'scripts', 'cmds', 'cache');
    const animeDataPath = resolve(cacheDir, 'anime.json');
    ...
    const animeData = [
      ...
      },
      {
        ...
        "hype": 4088,
        "description": "Living by the sea, protecting the sea, and journeying across the sea... They're the Blue Mermaids! Because of a shift in tectonic plates around 100 years ago, Japan lost much of its area to submergence. To preserve Japan's territories, coastal cities sprung up, one after the other. Eventually became marine cities, and along with the expansion of sea-lanes to connect them, the need arose for a sizable staff to protect the seas. At the same time, maritime jobs were becoming more popular among women. Hence, the Blue Mermaids, tasked with keeping the seas safe, had become every female student's dream job. This is when childhood friends Akeno Misaki and Moeka China enroll in a marine high school in Yokosuka, along with other girls who share their goal of becoming a Blue Mermaid. (Source: Aniplex of America) Episode 1 will be previewed at back-to-back screenings at the museum ship Mikasa in Yokosuka, Kanagawa Prefecture, on April 2, 2016. Regular broadcasting will begin on April 10, 2016.",
        "title": {
      ...
      },
      {
        ...
        "hype": 3240,
        "description": "Onigiri thrusts its players into mystical Japan, a land filled with mythical creatures of Japanese legend. Starting as a lone Oni, players fight against the malevolent influence of the Kamikui as their miasma spreads over the land. 8 NPCs, each with their own distinct personality and skills, ally themselves with the player to help build a legend, and quell the evil that has arisen. (Source: ANN)",
        "title": {
    ...
    try {
      ...
      // Write new anime data to file
      writeFileSync(animeDataPath, JSON.stringify(animeData, null, 2));
    } catch (error) {
