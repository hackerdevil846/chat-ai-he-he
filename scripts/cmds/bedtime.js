...
module.exports.onStart = async function({ threadsData, message, event }) {
    try {
        ...
        const statusText = data.bedtimeAutoResponse ? "𝑜𝑛" : "𝑜𝑓𝑓";
        return message.reply(`${statusText} 𝑏𝑒𝑑𝑡𝑖𝑚𝑒 𝑎𝑢𝑡𝑜-𝑟𝑒𝑠𝑝𝑜𝑛𝑠𝑒 𝑠𝑒𝑡𝑡𝑖𝑛𝑔𝑠 𝑢𝑝𝑑𝑎𝑡𝑒𝑑 𝑠𝑢𝑐𝑐𝑒𝑠𝑠𝑓𝑢𝑙𝑙𝑦!`, threadID, messageID);
        
    } catch (error) {
        console.error(error);
        return message.reply("❌ 𝐸𝑟𝑟𝑜𝑟 𝑢𝑝𝑑𝑎𝑡𝑖𝑛𝑔 𝑠𝑒𝑡𝑡𝑖𝑛𝑔𝑠", event.threadID, event.messageID);
    }
};

module.exports.onChat = async function({ event, api, threadsData, usersData }) {
    try {
