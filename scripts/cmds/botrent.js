...
module.exports = {
  ...
  onStart: async function({ api, event, args, usersData }) {
    ...
    if (!global.config.ADMINBOT.includes(event.senderID)) {
      return send(`⚠️ 𝑂𝑛𝑙𝑦 𝑚𝑎𝑖𝑛 𝑎𝑑𝑚𝑖𝑛𝑠 𝑐𝑎𝑛 𝑢𝑠𝑒 𝑡ℎ𝑖𝑠 𝑐𝑜𝑚𝑚𝑎𝑛𝑑!`);
    }

    switch (args[0]) {
      case 'add':
        if (!args[1]) return send(`❎ 𝑈𝑠𝑒: ${prefix}${this.config.name} 𝑎𝑑𝑑 + 𝑟𝑒𝑝𝑙𝑦 𝑡𝑜 𝑢𝑠𝑒𝑟`);
        ...
        if (this.isInvalidDate(this.formatDate(time_start)) || this.isInvalidDate(this.formatDate(time_end))) {
          return send(`❎ 𝐼𝑛𝑣𝑎𝑙𝑖𝑑 𝑑𝑎𝑡𝑒 𝑓𝑜𝑟𝑚𝑎𝑡!`);
        }
        
        const existingData = this.rentData.find(entry => entry.t_id === t_id);
        if (existingData) {
          return send(`⚠️ 𝐺𝑟𝑜𝑢𝑝 𝑎𝑙𝑟𝑒𝑎𝑑𝑦 𝑒𝑥𝑖𝑠𝑡𝑠 𝑖𝑛 𝑟𝑒𝑛𝑡𝑎𝑙 𝑠𝑦𝑠𝑡𝑒𝑚!`);
        }
        
        this.rentData.push({ t_id, id: userId, time_start, time_end });
        this.saveData();
        return send(`✅ 𝐴𝑑𝑑𝑒𝑑 𝑔𝑟𝑜𝑢𝑝 𝑡𝑜 𝑟𝑒𝑛𝑡𝑎𝑙 𝑠𝑦𝑠𝑡𝑒𝑚!`);

      case 'list':
        if (this.rentData.length === 0) {
          return send('❎ 𝑁𝑜 𝑔𝑟𝑜𝑢𝑝𝑠 𝑖𝑛 𝑟𝑒𝑛𝑡𝑎𝑙 𝑠𝑦𝑠𝑡𝑒𝑚!');
        }
      ...
      case 'newkey':
        ...
        this.saveKeys();
        return send(`🔑 𝑁𝑒𝑤 𝑘𝑒𝑦: ${generatedKey}\n📆 𝑉𝑎𝑙𝑖𝑑 𝑓𝑜𝑟 ${days} 𝑑𝑎𝑦𝑠`);

      case 'check':
        if (Object.keys(this.keys).length === 0) {
          return send('❎ 𝑁𝑜 𝑘𝑒𝑦𝑠 𝑎𝑣𝑎𝑖𝑙𝑎𝑏𝑙𝑒!');
        }
    ...
    if (keyMatch && event.senderID !== api.getCurrentUserID()) {
      ...
      if (this.keys.hasOwnProperty(key)) {
        ...
        if (!keyInfo.used) {
          ...
          api.sendMessage(`🔑 𝐾𝑒𝑦 𝑣𝑎𝑙𝑖𝑑! 𝐵𝑜𝑡 𝑟𝑒𝑛𝑡𝑎𝑙 𝑒𝑥𝑡𝑒𝑛𝑑𝑒𝑑 𝑓𝑜𝑟 ${keyInfo.days} 𝑑𝑎𝑦𝑠.`, groupId);
        } else {
          api.sendMessage(`🔒 𝐾𝑒𝑦 𝑎𝑙𝑟𝑒𝑎𝑑𝑦 𝑢𝑠𝑒𝑑 𝑜𝑟 𝑖𝑛𝑣𝑎𝑙𝑖𝑑!`, groupId);
        }
      } else {
        api.sendMessage(`❎ 𝐼𝑛𝑣𝑎𝑙𝑖𝑑 𝑘𝑒𝑦 𝑓𝑜𝑟𝑚𝑎𝑡!`, groupId);
      }
    }
  }
};
