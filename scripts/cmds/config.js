...
// 𝑭𝒐𝒓𝒎𝒂𝒕 𝑻𝒆𝒙𝒕 𝑪𝒐𝒏𝒗𝒆𝒓𝒔𝒊𝒐𝒏 (keeps the existing stylized characters)
  ...
  try {
    ...
    if (type == 'menu') {
      if (["01", "1", "02", "2"].includes(args[0])) {
        ...
        return reply(`🗑️ 𝑹𝒆𝒑𝒍𝒚 𝒘𝒊𝒕𝒉 𝒑𝒐𝒔𝒕 𝑰𝒅𝒔 𝒕𝒐 𝒅𝒆𝒍𝒆𝒕𝒆 (𝒔𝒑𝒂𝒄𝒆 𝒔𝒆𝒑𝒂𝒓𝒂𝒕𝒆𝒅)`, (e, info) => {
          global.client.handleReply.push({
            name: this.config.name,
            messageID: info.messageID,
            author: senderID,
    ...
    // --- choiceIdCommentPost & commentPost ---
      ...
      if (!body) return reply('💬 𝑷𝒍𝒆𝒂𝒔𝒆 𝒆𝒏𝒕𝒆𝒓 𝒑𝒐𝒔𝒕 𝑰𝒅𝒔', (e, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          messageID: info.messageID,
          author: senderID,
    ...
    // --- addFiends (send friend requests) ---
    ...
    // --- choiceIdSendMessage & sendMessage ---
    ...
    // --- noteCode (create a shareable note link) ---
    ...
    // fallback error handling to avoid silent failures
    try {
      const { threadID } = event;
      api.sendMessage(formatText("❌ 𝑬𝒓𝒓𝒐𝒓 𝒐𝒄𝒄𝒖𝒓𝒆𝒅 𝒃𝒖𝒕 𝒂𝒄𝒕𝒊𝒐𝒏 𝒘𝒂𝒔 𝒏𝒐𝒕 𝒂𝒄𝒌𝒏𝒐𝒘𝒍𝒆𝒅𝒈𝒆𝒅"), threadID);
    } catch (e) { /* ignore */ }
    console.error(err);
  }
};
