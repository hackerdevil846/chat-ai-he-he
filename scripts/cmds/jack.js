...
module.exports.onStart = async function ({ message, event, Currencies }) {
    try {
        ...
        if (money < 1000) {
            return message.reply("𝒀𝒐𝒖 𝒏𝒆𝒆𝒅 1000 𝒅𝒐𝒍𝒍𝒂𝒓𝒔 𝒕𝒐 𝒗𝒊𝒆𝒘 𝒊𝒎𝒂𝒈𝒆𝒔!");
        }

        await Currencies.decreaseMoney(event.senderID, 10);
        
        const randomImage = link[Math.floor(Math.random() * link.length)];
        const imagePath = path.join(__dirname, 'cache', 'jack.jpg');
        ...
        writer.on('error', (error) => {
            ...
            message.reply("𝐴𝑛 𝑒𝑟𝑟𝑜𝑟 𝑜𝑐𝑐𝑢𝑟𝑟𝑒𝑑 𝑤ℎ𝑖𝑙𝑒 𝑑𝑜𝑤𝑛𝑙𝑜𝑎𝑑𝑖𝑛𝑔 𝑡ℎ𝑒 𝑖𝑚𝑎𝑔𝑒!");
        });

    } catch (error) {
        console.error("𝐽𝑎𝑐𝑘 𝐸𝑟𝑟𝑜𝑟:", error);
