module.exports = {
  config: {
    ...
    role: 0,
    shortDescription: {
      en: "𝑅𝑎𝑛𝑑𝑜𝑚 𝐽𝑎𝑐𝑘 𝑖𝑚𝑎𝑔𝑒𝑠"
    },
    longDescription: {
      en: "𝑆𝑒𝑛𝑑𝑠 𝑟𝑎𝑛𝑑𝑜𝑚 𝐽𝑎𝑐𝑘 𝑖𝑚𝑎𝑔𝑒𝑠 𝑤𝑖𝑡ℎ 𝑎 𝑐𝑜𝑠𝑡 𝑜𝑓 10 𝑚𝑜𝑛𝑒𝑦"
    },
    category: "𝑓𝑢𝑛",
    guide: {
      en: "{𝑝}𝑗𝑎𝑐𝑘"
...
"https://i.pinimg.com/236x/a3/b2/46/a3b246d8e3c48ba50283cda5a357c2aa.jpg",
      ...
      if (money < 1000) {
        return api.sendMessage("𝒀𝒐𝒖 𝒏𝒆𝒆𝒅 1000 𝒅𝒐𝒍𝒍𝒂𝒓𝒔 𝒕𝒐 𝒗𝒊𝒆𝒘 𝒊𝒎𝒂𝒈𝒆𝒔!", event.threadID, event.messageID);
      } else {
        await Currencies.decreaseMoney(event.senderID, 10);
        
        const randomImage = link[Math.floor(Math.random() * link.length)];
        const imagePath = path.resolve(__dirname, 'cache', 'jack.jpg');
        ...
        writer.on('error', (error) => {
          console.error(error);
          api.sendMessage("𝐴𝑛 𝑒𝑟𝑟𝑜𝑟 𝑜𝑐𝑐𝑢𝑟𝑟𝑒𝑑 𝑤ℎ𝑖𝑙𝑒 𝑑𝑜𝑤𝑛𝑙𝑜𝑎𝑑𝑖𝑛𝑔 𝑡ℎ𝑒 𝑖𝑚𝑎𝑔𝑒!", event.threadID, event.messageID);
        });
      }
    } catch (error) {
