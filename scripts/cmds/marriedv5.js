...
async function makeImage({ one, two }) {
    ...
    let married_img = await jimp.read(__root + "/marriedv5.png");
    ...
    married_img.composite(circleOne.resize(130, 130), 300, 150).composite(circleTwo.resize(130, 130), 170, 230);
...
module.exports.run = async function ({ event, api, args }) {    
    ...
    if (!mention[0]) {
        ...
        return makeImage({ one, two }).then(path => {
            api.sendMessage({ 
                body: `💒 𝑪𝒐𝒏𝒈𝒓𝒂𝒕𝒔! ${event.mentions[two].replace("@", "")} 𝒂𝒓 ${event.senderID == two ? "𝒕𝒖𝒎𝒊 𝒏𝒊𝒋𝒆𝒓 𝒃𝒊𝒚𝒆 𝒌𝒐𝒓𝒍𝒆? 🤔" : "𝒂𝒑𝒏𝒊 𝒃𝒊𝒚𝒆 𝒉𝒐𝒍𝒐! 💖"}\n━━━━━━━━━━━━━━━\n💖 𝑷𝒐𝒘𝒆𝒓𝒆𝒅 𝒃𝒚 𝑨𝒔𝒊𝒇 𝑴𝒂𝒉𝒎𝒖𝒅`,
