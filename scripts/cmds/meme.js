...
module.exports = {
    config: {
        ...
        author: "𝐀𝐬𝐢𝐟 𝐌𝐚𝐡𝐦𝐮𝐝",
        shortDescription: {
            en: "𝐑𝐚𝐧𝐝𝐨𝐦 𝐦𝐞𝐦𝐞 𝐢𝐦𝐚𝐠𝐞"
        },
        longDescription: {
            en: "𝐆𝐞𝐧𝐞𝐫𝐚𝐭𝐞𝐬 𝐫𝐚𝐧𝐝𝐨𝐦 𝐦𝐞𝐦𝐞 𝐢𝐦𝐚𝐠𝐞𝐬 𝐟𝐫𝐨𝐦 𝐚 𝐯𝐚𝐬𝐭 𝐜𝐨𝐥𝐥𝐞𝐜𝐭𝐢𝐨𝐧"
        },
        category: "𝐢𝐦𝐚𝐠𝐞",
        guide: {
            en: "{p}meme"
    ...
    onStart: async function({ message }) {
        // Dependency check to prevent crash if modules are missing
        try {
            require("axios");
            require("fs-extra");
        } catch (e) {
        ...
        // Full list of links provided (No links removed or edited)
        const memeLinks = [
        ...
        const loadingMsg = await message.reply("⏳ 𝐅𝐢𝐧𝐝𝐢𝐧𝐠 𝐚 𝐟𝐮𝐧𝐧𝐲 𝐦𝐞𝐦𝐞...");

        // Retry logic: try up to 3 times if a link is dead (404)
        let attempts = 0;
        const maxAttempts = 3;
        let success = false;
        
        // Define temporary file path (OS independent)
        const tempFilePath = path.join(os.tmpdir(), `meme_${Date.now()}.jpg`);
        ...
        while (attempts < maxAttempts && !success) {
            ...
            try {
                // Download image
                const response = await axios.get(randomMeme, { 
                    responseType: "arraybuffer",
                    headers: {
                        'User-Agent': 'Mozilla/5.0' // Helps avoid some server blocks
                    }
                });
                
                // Write to temp file
                await fs.writeFileSync(tempFilePath, Buffer.from(response.data, "binary"));
                ...
                // Send the meme
                await message.reply({
                    body: `🤣 𝐑𝐚𝐧𝐝𝐨𝐦 𝐌𝐞𝐦𝐞 𝐁𝐨𝐭\n━━━━━━━━━━━━━━━━━━\n✨\n💫 𝐓𝐨𝐭𝐚𝐥 𝐌𝐞𝐦𝐞𝐬: ${memeLinks.length} 𝐏𝐡𝐨𝐭𝐨𝐬\n🎉 𝐄𝐧𝐣𝐨𝐲 𝐘𝐨𝐮𝐫 𝐌𝐞𝐦𝐞!`,
                ...
                // If it fails, the loop continues and picks a NEW random link
            }
        }
        ...
        // Final error message if all 3 attempts failed
        if (!success) {
            if (loadingMsg && loadingMsg.messageID) await message.unsendMessage(loadingMsg.messageID);
            return message.reply("⚠️ 𝐒𝐨𝐫𝐫𝐲, 𝐜𝐨𝐮𝐥𝐝 𝐧𝐨𝐭 𝐟𝐞𝐭𝐜𝐡 𝐚 𝐦𝐞𝐦𝐞 𝐫𝐢𝐠𝐡𝐭 𝐧𝐨𝐰. 𝐏𝐥𝐞𝐚𝐬𝐞 𝐭𝐫𝐲 𝐚𝐠𝐚𝐢𝐧!");
        }
    }
};
