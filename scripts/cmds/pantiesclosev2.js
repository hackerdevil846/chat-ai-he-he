...
module.exports = {
  config: {
    ...
    longDescription: {
      en: "𝐺𝑒𝑛𝑒𝑟𝑎𝑡𝑒𝑠 𝑟𝑎𝑛𝑑𝑜𝑚 𝑝𝑎𝑛𝑡𝑖𝑒𝑠 𝑐𝑙𝑜𝑠𝑒-𝑢𝑝 𝑖𝑚𝑎𝑔𝑒𝑠"
    },
    guide: {
      en: "{p}pantiesclosev2"
    },
  ...
  onStart: async function ({ api, event, message }) {
    try {
      // 1. Full list of links (EXACTLY AS PROVIDED, NO CHANGES)
      const links = [
      ...
      // 3. Prepare paths and filenames
      const cacheDir = path.join(__dirname, "cache");
      const fileName = `panties_${Date.now()}.jpg`;
      ...
      // 5. Download Image with Stream (Safe for all file sizes)
      ...
      console.error("PantiesCloseV2 Error:", error);
