    ...
    try {
      ...
      const images = await Promise.all(
        urls.map(async (url, index) => {
          ...
          const imagePath = path.join(cacheFolderPath, `quickgen_${index + 1}.jpg`);
      ...
      const combinedImagePath = path.join(cacheFolderPath, `quickgen_combined.jpg`);
