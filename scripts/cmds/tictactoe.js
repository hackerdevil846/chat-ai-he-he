...
async function displayBoard(data) {
    ...
    var quanO = await loadImage("https://i.postimg.cc/rFP6xCLXQ/O.png");
    ...
    for (var i = 0; i < 3; i++) {
        for (var j = 0; j < 3; j++) {
            ...
            if (temp == "1") {
                if (data.isX) {
                    cc.drawImage(quanO, x, y, 360, 360);
                } else {
                    cc.drawImage(quanX, x, y, 360, 360);
                }
            }
            if (temp == "2") {
                if (data.isX) {
...
module.exports.handleReply = async function ({ event, api, handleReply }) {
    ...
    if (!isNaN(number) && number > 0 && number < 10) {
        ...
        var col;
        if (number == 1 || number == 4 || number == 7) col = 0;
        if (number == 2 || number == 5 || number == 8) col = 1;
        if (number == 3 || number == 6 || number == 9) col = 2;
        ...
        if (checkGameOver(data)) {
            ...
            if (checkAIWon(data)) {
                lmao = `𝑻𝒖𝒎𝒊 𝒉𝒂𝒓𝒍𝒆! ${gayban[Math.floor(Math.random() * gayban.length)]}𝒄𝒉𝒊𝒄𝒌𝒆𝒏 🐔`;
            } else {
                if (checkPlayerWon(data)) lmao = "𝑻𝒖𝒎𝒊 𝒋𝒊𝒕𝒍𝒆! :<";
                else lmao = "𝑫𝒓𝒂𝒘 𝒉𝒐𝒊𝒔𝒆!";
            }
            global.client.handleReply.delete(threadID);
        }
        
        var msg = lmao !== "𝒄𝒉𝒊𝒄𝒌𝒆𝒏 🐔" ? lmao : temp == undefined ? "𝑪𝒉𝒆𝒄𝒌 𝒌𝒐𝒓𝒂𝒓 𝒋𝒐𝒏𝒏𝒐 𝒄𝒆𝒍𝒍 𝒆𝒓 𝒏𝒖𝒎𝒃𝒆𝒓 𝒓𝒆𝒑𝒍𝒚 𝒅𝒂𝒐" : temp;
        api.sendMessage({
            body: msg,
...
module.exports.run = async function ({ event, api, args }) {
    ...
    if (!data.gameOn) {
        ...
        global.client.handleReply.set(threadID, newData);
    } else {
        return api.sendMessage("𝑬𝒊 𝒈𝒓𝒐𝒖𝒑 𝒆 𝒂𝒈𝒆 𝒕𝒉𝒆𝒌𝒆 𝒄𝒉𝒆𝒔𝒔𝒃𝒐𝒂𝒓𝒅 𝒂𝒄𝒉𝒆\n𝑩𝒂𝒃𝒐𝒉𝒂𝒓 𝒌𝒐𝒓𝒐:\n" + concak + " 𝒄𝒐𝒏𝒕𝒊𝒏𝒖𝒆 -> 𝒄𝒐𝒏𝒕𝒊𝒏𝒖𝒆 𝒌𝒐𝒓𝒐\n" + concak + " 𝒅𝒆𝒍𝒆𝒕𝒆 -> 𝒆𝒓𝒂𝒔𝒆 𝒌𝒐𝒓𝒐", threadID, messageID);
    }
};
