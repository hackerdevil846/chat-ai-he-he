...
async function endS(api, event, getText, Users, Currencies, object) {
  ...
  for (let i = 0; i < object.players - 1; i++) {
    ...
    if (result == "𝐷𝑟𝑎𝑤") await increaseMoney(playerD.id, playerD.bet);
    else if (result.slice(0,3) != "𝐿𝑜𝑠") await increaseMoney(playerD.id, playerD.bet*(bonus + 1));
...
module.exports.onChat = async function({ event, api, getText, Users, Currencies }) {
  ...
  if (threadID in global.client.xidach_otm) {
    ...
    if (body == "get") {
      ...
      global.client.xidach_otm[threadID].data[curU].cards.forEach(c => {
        atms.push(fs.createReadStream(__dirname + `/poker/${module.exports.cards[c]}`));
      });
      api.sendMessage({
...
module.exports.onStart = async function({ api, event, args, getText, Users, Currencies }) {
  // Check dependencies
  try {
  ...
  if (global.configModule[module.exports.config.name].maxPlayers < 2 || global.configModule[module.exports.config.name].maxPlayers > 10) {
    global.configModule[module.exports.config.name].maxPlayers = 5;
  }
  ...
  switch (args[0]) {
    case 'create':
      ...
      if (threadID in global.client.xidach_otm) return send(getText("alreadyHave"));
      ...
      global.client.xidach_otm[threadID] = {
        ...
        bonus: global.configModule[module.exports.config.name]
      };
      return sendTC(getText("openSuccess", global.configModule[module.exports.config.name].maxPlayers, prefix+module.exports.config.name));
      
    case "join":
      if (!(threadID in global.client.xidach_otm)) return sendT(getText("noGame"));
      if (global.client.xidach_otm[threadID].data.find(p => p.id == senderID)) return send(getText("alreadyJoined"));
      if (global.client.xidach_otm[threadID].players == global.configModule[module.exports.config.name].maxPlayers) return send(getText("out_of_room"));
      if (global.client.xidach_otm[threadID].status == "started") return send(getText("alreadyStarted_1"));
      ...
      return sendC(getText("joinSuccess", global.client.xidach_otm[threadID].players, global.configModule[module.exports.config.name].maxPlayers));
    ...
    case "leave":
      ...
      if (global.client.xidach_otm[threadID].status == "pending") {
        const playerIndex = global.client.xidach_otm[threadID].data.findIndex(p => p.id == senderID);
        if (playerIndex !== -1) {
          await increaseMoney(senderID, global.client.xidach_otm[threadID].data[playerIndex].bet);
          global.client.xidach_otm[threadID].data.splice(playerIndex, 1);
        }
      } else {
        global.client.xidach_otm[threadID].data = global.client.xidach_otm[threadID].data.filter(p => p.id != senderID);
      }
      
      sendC(getText("outSuccess", global.client.xidach_otm[threadID].players, global.configModule[module.exports.config.name].maxPlayers));
      
      if (global.client.xidach_otm[threadID].players == 1) {
        delete global.client.xidach_otm[threadID];
        return sendC(getText("only_bot_left"));
      }
      break;
      
    case "start":
      var cardKeys = Object.keys(module.exports.cards);
      ...
      if (!(threadID in global.client.xidach_otm)) return sendT(getText("noGame"));
      const playerData = global.client.xidach_otm[threadID].data.find(p => p.id == senderID);
      if (!playerData || playerData.type != "author") return send(getText("not_author"));
      ...
      await sendTC(getText("testInbox"));
      for (let i = 0; i < global.client.xidach_otm[threadID].data.length; i++) {
        ...
        try {
          ...
          let curName = await Users.getNameUser(p.id) || "User";
      ...
      await sendTC(getText("checkInbox_noti"));
      for (let i = 0; i < global.client.xidach_otm[threadID].data.length; i++) {
        try {
          let p = global.client.xidach_otm[threadID].data[i];
          ...
          var atms = [];
          atms.push(fs.createReadStream(__dirname + `/poker/${module.exports.cards[one]}`));
          atms.push(fs.createReadStream(__dirname + `/poker/${module.exports.cards[two]}`));
          if (p.id == api.getCurrentUserID()) continue;
          await api.sendMessage({
      ...
      await sendTC(getText("explaining"));
      await delay(1000);
      await sendT(getText("start_after_5s"));
      await delay(5000);
      await sendT(getText("started"));
      await delay(300);
    ...
    default:
      return sendT(getText("xidachRules"));
  }
};
